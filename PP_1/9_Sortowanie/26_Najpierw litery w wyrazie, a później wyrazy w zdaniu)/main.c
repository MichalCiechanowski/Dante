...
{
    int word_len = word_length(word);

    for (int i = 0; i < word_len - 1; i++)
       for (int j = 0; j < word_len - i - 1; j++)
           if (char_cmp(word[j], word[j + 1]) > 0) {
               char tmp = word[j];
               word[j] = word[j + 1];
               word[j + 1] = tmp;
           }
...
int sort_letters_and_words_alphabetically(char tab[])
{
    if (tab == NULL)
        return 1;
...
{
    char tab[1001] = {0};
    char tab2[1001] = "To jest przykladowy tekst";
    sort_letters_and_words_alphabetically(tab2);

    printf("Podaj tekst:\n");
    scanf("%1000[^\n]s", tab);

    int err_code = sort_letters_and_words_alphabetically(tab);
